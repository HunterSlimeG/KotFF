...
var skills_path1 = "user://Saves/File1/Skills.json"
var skills_path2 = "user://Saves/File2/Skills.json"
var skills_path3 = "user://Saves/File3/Skills.json"
var skills_paths = [skills_path1, skills_path2, skills_path3]
...
func create_file():
	...
	for i in stats_paths:
		if not FileAccess.file_exists(i):
			...
			pass
	for j in skills_paths:
		if not FileAccess.file_exists(j):
			var file = FileAccess.open("res://Scripts/Data/Default_Skills.json", FileAccess.READ)
			var content = json.parse_string(file.get_as_text())
			data = content
			save_data(data, j)
		else:
